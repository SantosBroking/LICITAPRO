...
export default function App() {
  ...
  const projDetailView = currentProject
    ? h(ProjectDetail, { project: isAdmin?currentProject:visibleCurrentProject, vehicles: isAdmin?vehicles:visibleVehicles, companies, config, projects: isAdmin?projects:visibleProjects, onSaveConfig:handleSaveConfig, onSaveCompany:async c=>{ const ex=companies.find(x=>x.id===c.id||x.rfc===c.rfc); setCompanies(ex?companies.map(x=>(x.id===c.id||x.rfc===c.rfc)?{...x,...c}:x):[...companies,c]); try{ const guardada = await saveCompanyViaEndpoint(c); setCompanies(prev => prev.map(x=>x.id===guardada.id?guardada:x)); log(user, ex?'actualizó':'creó', 'empresa', c.id, c.name); }catch(e){ console.error('[2E3B] Error guardando empresa vía /api/save-company:', e); } }, onUpdate:upProject, onSave:handleSaveProject, onDelete:handleDeleteProject, onNav:nav, user, logFn:log, activeTab:effectiveProjTab, setActiveTab:setProjTab, cotSubTab:effectiveSubTabs.cotizacion, setCotSubTab:(val)=>setSubTab('cotizacion', val), operacionSubTab:effectiveSubTabs.operacion, setOperacionSubTab:(val)=>setSubTab('operacion', val), docsSubTab:effectiveSubTabs.docs, setDocsSubTab:(val)=>setSubTab('docs', val) })
  ...
  const content = ({
    ...
    project_new:    h(ProjectForm,   { companies, config, user, projects: isAdmin?projects:visibleProjects, onSaveConfig:handleSaveConfig, onSave:handleSaveProject, onCancel:()=>nav('projects') }),
