...
export default function CotizacionTab({ project, onUpdate, activeTab, setActiveTab, config, onSaveConfig }) {
  ...
  return h('div', null,
    ...
    tab==='equipo' && h('div', null,
      // Panel: cotización de solo equipamiento
      h('div', { className:'card', style:{ marginBottom:12, borderLeft:'3px solid '+(cot.soloEquipo?'#9B7EDE':'var(--b2)') } },
        h('label', { style:{ display:'flex', alignItems:'center', gap:10, cursor:'pointer' } },
          h('input', { type:'checkbox', checked:cot.soloEquipo, onChange:e=>updCot({...cot, soloEquipo:e.target.checked}), style:{ width:18, height:18, cursor:'pointer' } }),
          h('div', null,
            h('div', { style:{ fontSize:13, fontWeight:600 } }, 'Cotización de solo equipamiento'),
