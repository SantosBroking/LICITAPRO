...
Estado: v1 (QA correctivos LP-EMIT-001R / LP-EMIT-001S / LP-EMIT-001C aplicados) — SOLO DISEÑO. No implementado. No hay SQL nuevo, no hay migraciones, no hay Supabase, no hay UI/API. Sin staging, sin commit, sin push.
Repositorio: `SantosBroking/LICITAPRO` · Branch base conceptual: `feat/licita-engine-v1`
HEAD remoto confirmado por Control Tower: `0a94a09baa62dc5ecea2927cad6ca063cde0ca64`
...
> Este documento es un contrato de operación, no una implementación. Ningún endpoint ni módulo de aplicación descrito aquí existe todavía. Es la especificación exacta que una misión posterior de implementación debe cumplir, con la aprobación explícita de Control Tower — consistente con el canon ya validado por el capability spike de LP-EMIT-003/LP-EMIT-003R (`server/emission/transactionCoordinator.js`): PostgreSQL/`LP-SCHEMA-002` provee el schema, los invariantes y los helpers SQL (`validate_quote_for_emission`, `fn_next_quote_version_number`, `fn_supersede_previous_quote_versions`, triggers de inmutabilidad); la capa de aplicación en Node coordina, sosteniendo una única sesión/transacción de principio a fin, la secuencia completa de §7 — nunca reimplementa esos invariantes ni el cálculo financiero; `calculateQuoteDraft` permanece en JavaScript puro, ejecutado dentro de esa misma transacción abierta, nunca movido a SQL; `quote-core`/`pricingEngine.js` permanecen puros, sin acceso a base de datos. **No** se trata, entonces, de una única función/procedimiento SQL invocado por un handler que le delega toda la operación de emisión.
...
- La **atomicidad** se especifica como una secuencia lógica de 14 pasos dentro de una sola transacción/sesión, con **rollback total ante fallos PRE-COMMIT** (LP-EMIT-PUBLISH-001, corrección B); un fallo, error o pérdida de respuesta del `COMMIT` en sí puede producir `commit_outcome = UNKNOWN` en lugar de un rollback demostrable, conforme §7/§9.2A — sin diseñar el SQL concreto (eso pertenece a una migración futura de `LP-SCHEMA-002`/una nueva misión de implementación).
...
Toda la secuencia siguiente ocurre dentro de **una** transacción/sesión de base de datos (ver §11, boundary de implementación). No se especifica aquí el dialecto SQL exacto — eso es una decisión de la migración de implementación. **Orden corregido (LP-EMIT-001R, corrección 5):** el cálculo financiero y la validación/selección de su resultado ocurren **antes** de construir cualquier snapshot comercial — los snapshots se construyen una sola vez, ya con el mapping de identidad y la selección de supplemental resueltos, nunca antes.

**Dos desenlaces distintos, no uno solo (LP-EMIT-001C, corrección 3 — vinculante):**

- **A. Fallos en los pasos 1-13** (mutex no adquirido, precondición de lifecycle, `validate_quote_for_emission`, rechazo financiero, fallo de invariante interno, inconsistencia comercial de supplemental, snapshot incompleto, o cualquier fallo de infraestructura antes de enviar `COMMIT`): la transacción se **aborta** y se ejecuta `ROLLBACK` — este es el caso "rollback total" ya descrito en versiones previas de este contrato: **ningún efecto parcial debe sobrevivir a un rollback efectivo**, y el resultado termina con uno de los errores de §9.
- **B. El `COMMIT` del paso 14 responde satisfactoriamente:** la emisión es **`COMMITTED`** — `quote_version`/`quote_version_calculation` existen de forma durable, `quote.status` transicionó según corresponda, y el resultado exitoso de §9.3 se retorna normalmente.
- **C. El `COMMIT` del paso 14 lanza un error, o su respuesta se pierde (p. ej. la conexión se corta mientras se esperaba la confirmación de PostgreSQL):** el desenlace real de la transacción es **indeterminado** — PostgreSQL puede haber completado el `COMMIT` antes de que la falla ocurriera, o no. Este caso **no** es equivalente al caso A y **no** se resuelve asumiendo que "no se guardó nada": ver el tratamiento explícito al final de este §7 y en §9.2A.

No es correcto afirmar que un `ROLLBACK` best-effort posterior al fallo de `COMMIT` demuestra que el `COMMIT` no ocurrió — un `ROLLBACK` enviado a una sesión cuyo `COMMIT` anterior en realidad sí se completó en el servidor no revierte nada (la transacción ya no existe) y no aporta evidencia sobre el desenlace real.
...
10. **Crear la nueva `quote_version`**, inicialmente `status='ISSUED'` (único estado permitido en `INSERT` por `fn_quote_version_content_and_status_guard`, LP-SCHEMA-002), con los snapshots del paso 8, `commercial_snapshot_schema_version='v1'`, y `issued_at = <emission_timestamp>` (§7.1). **La capa de emisión captura el `new_version_id` retornado por este `INSERT` y lo conserva fuera del resultado transaccional inmediato** (LP-EMIT-001C, corrección 4 — nueva) — de modo que ese identificador siga disponible como ancla de reconciliación incluso si el round-trip del `COMMIT` del paso 14 falla o se pierde (caso C, más abajo). Esto no requiere ninguna columna nueva, ningún idempotency token, ni ninguna modificación de schema — es, simplemente, que el llamador ya tiene ese UUID en memoria desde el momento en que este `INSERT` retorna, y no lo descarta solo porque el paso 14 pudiera fallar después.
...
14. **Commit** — todos los pasos 9-13 se confirman como una sola unidad. Tres desenlaces posibles (ver el preámbulo de este §7 y §9.2A):
    - Si cualquier paso 1-13 falla (antes de siquiera enviar `COMMIT`): **rollback total lógico**, sin dejar ninguna `quote_version` sin su `quote_version_calculation`, sin `version_number` "saltado" de forma visible a otra transacción, y sin cambio de `quote.status`.
    - Si el `COMMIT` responde satisfactoriamente: emisión **`COMMITTED`** — resultado exitoso (§9.3).
    - Si el `COMMIT` en sí lanza un error o su respuesta se pierde: **`commit_outcome = UNKNOWN`** (LP-EMIT-001C, corrección 3/4 — nueva). Puede intentarse un `ROLLBACK` best-effort inmediatamente después, pero un `ROLLBACK` posterior exitoso **no** convierte `UNKNOWN` en "no comprometido" — se preserva la causa original del fallo de `COMMIT`, y si ese `ROLLBACK` best-effort también falla, se preserva además su error (`rollbackError`) sin que reemplace la causa original. No hay retry automático de `COMMIT` ni reemisión automática de la `quote` en este punto.
      **Regla de reconciliación posterior (LP-EMIT-001C, corrección 4):** cuando `commit_outcome = UNKNOWN`, una **nueva** sesión/transacción — fuera de la transacción original, ver §11.1 — puede consultar por el `new_version_id` capturado en el paso 10. Si esa consulta encuentra la `quote_version` visible **y** su `quote_version_calculation` 1:1 también existe, eso **confirma positivamente** que la emisión sí se comprometió. Si el `new_version_id` **no** aparece de inmediato en esa consulta de reconciliación, **no** se asume automáticamente que el `COMMIT` no se completó — la ausencia inicial por sí sola no demuestra que el `COMMIT` original no pueda estar todavía resolviéndose del lado del servidor. Mientras no exista esa confirmación positiva, o una resolución operativa explícita (fuera de alcance de este contrato), el resultado permanece `UNKNOWN`. **No se permite** disparar un retry o una reemisión automática de la `quote` basándose únicamente en una ausencia inicial en la consulta de reconciliación.
...
| `PERSISTENCE_TRANSACTION_FAILURE` | Cualquier fallo de infraestructura durante los pasos 9-14 de §7 (conexión perdida, deadlock, violación de constraint inesperada) — exclusivamente fallo de DB/transacción/infraestructura durante persistencia. Incluye tanto los fallos pre-`COMMIT` (paso A del §7) como el caso `commit_outcome = UNKNOWN` (paso C del §7) — ver §9.2A para el tratamiento diferenciado dentro de esta misma categoría externa | Preserva el error de infraestructura original de la capa de persistencia; ver §9.2A para el caso `commit_outcome = UNKNOWN` |

### 9.2A `PERSISTENCE_TRANSACTION_FAILURE` — distinción interna pre-COMMIT vs. COMMIT indeterminado (LP-EMIT-001C, corrección 5 — nueva)

`PERSISTENCE_TRANSACTION_FAILURE` sigue siendo el **único** código canónico externo para fallos de infraestructura de persistencia — este contrato **no** introduce un código externo nuevo. Dentro de esa misma categoría, se distingue:

- **Fallos pre-`COMMIT`** (cualquier fallo de infraestructura en los pasos 9-13 de §7, antes de que `COMMIT` siquiera se envíe): terminan mediante `ROLLBACK` normal — el desenlace es inequívoco, la transacción no se comprometió.
- **Fallo durante el round-trip del `COMMIT` del paso 14** (caso C de §7): el resultado reportado bajo `PERSISTENCE_TRANSACTION_FAILURE` **debe preservar explícitamente**:
  - `commit_outcome = UNKNOWN` — nunca `NOT_COMMITTED`, nunca `ROLLED_BACK`, y nunca simplemente omitido;
  - la causa original del fallo de `COMMIT` (`cause`), sin reinterpretarla ni reducirla a un mensaje genérico;
  - `rollbackError`, cuando el `ROLLBACK` best-effort posterior (§7, caso C) también haya fallado — adjunto, sin reemplazar la causa original.
- `commit_outcome = UNKNOWN` **no debe ocultarse** bajo un mensaje genérico de `PERSISTENCE_TRANSACTION_FAILURE` indistinguible del caso pre-`COMMIT` — quien consuma este error debe poder distinguir "se sabe que no se comprometió" de "se desconoce si se comprometió".
- **Nunca** se dispara un retry automático de la operación de emisión cuando `commit_outcome = UNKNOWN` — ver la regla de reconciliación de §7 paso 14.

Esta distinción es **interna** a la categoría `PERSISTENCE_TRANSACTION_FAILURE` — no crea el código externo `EMISSION_COMMIT_OUTCOME_UNKNOWN`; ese código (usado en el capability spike de LP-EMIT-003/LP-EMIT-003R, `server/emission/transactionCoordinator.js`) pertenece exclusivamente al coordinator interno de esa implementación, no al contrato externo de LP-EMIT-001.
...
**Excepción explícita — reconciliación posterior de un `commit_outcome = UNKNOWN` (LP-EMIT-001C, corrección 6 — nueva):** la consulta de reconciliación descrita en §7 paso 14 (buscar el `new_version_id` capturado en el paso 10, en una sesión/transacción **nueva**, distinta de la original) **no** viola el boundary de una sola sesión/transacción descrito arriba. Esa consulta ocurre necesariamente después de que la transacción original ya terminó (con éxito, con fallo, o con desenlace indeterminado) — no continúa la emisión, no reintenta ningún paso de la secuencia de §7, y no modifica nada: **solo verifica** si el resultado durable de una emisión cuyo `COMMIT` tuvo un desenlace indeterminado terminó existiendo o no. El boundary de "una misma sesión/transacción de principio a fin" sigue siendo íntegro para la operación de emisión en sí misma; esta excepción se limita, exclusivamente, a la verificación posterior de un resultado ya indeterminado.
...
LP-EMIT-001C cerró una tercera área que la versión anterior dejaba tratada de forma incompleta, sin que fuera una ambigüedad genuina — solo faltaba nombrarla explícitamente:

- **Desenlace indeterminado de `COMMIT`:** ya NO se trata el fallo de `COMMIT` como equivalente a "no comprometido" — §7 (preámbulo y paso 14) y §9.2A cierran explícitamente que un `COMMIT` que lanza error o pierde su respuesta produce `commit_outcome = UNKNOWN`, distinto tanto de `COMMITTED` como de cualquier fallo pre-`COMMIT` resuelto por `ROLLBACK` normal; un `ROLLBACK` best-effort posterior nunca convierte `UNKNOWN` en "no comprometido", y la reconciliación vía el `new_version_id` capturado en el paso 10 (§7, §11.1) es la única vía contractual para resolver esa incertidumbre — nunca un retry/reemisión automática basado solo en una ausencia inicial. Esto sigue reportándose bajo el único código externo canónico `PERSISTENCE_TRANSACTION_FAILURE` (§9.2A) — no se crea ningún código externo nuevo; `EMISSION_COMMIT_OUTCOME_UNKNOWN` permanece como detalle interno del coordinator del spike LP-EMIT-003/LP-EMIT-003R, no como parte del contrato externo de LP-EMIT-001.

No se identificó ninguna ambigüedad genuina nueva en el canon existente (LP-SCHEMA-001 v1.3, LP-SCHEMA-002T, LP-ORCH-001, `pricingEngine.js` LP-ENG-002T, y el capability spike de LP-EMIT-003/LP-EMIT-003R) que este contrato no pudiera resolver. `DECISION_REQUIRED: 0`.

---

**LP-EMIT-001 v1 (LP-EMIT-001R / LP-EMIT-001S / LP-EMIT-001C aplicados) — fin de entrega.** Solo diseño. Sin SQL nuevo, sin modificación de SQL existente, sin Supabase, sin migraciones, sin UI, sin API, sin staging, sin commit, sin push, sin PR, sin merge, sin deploy.
