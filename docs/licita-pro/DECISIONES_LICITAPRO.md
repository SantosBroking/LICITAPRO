...
- **No toda Orden de Compra requiere firma.** Se agregó el modelo de `requiereFirma` (booleano, default `true` por compatibilidad legacy) y `estadoOperativo` (derivado, no destructivo) para distinguir OCs que van a firma de las que solo quedan como soporte documental/expediente. **Una OC archivada como "solo expediente" no cuenta como pendiente** — el estado operativo (borrador/emitida/enviada al proveedor/en firma/firmada/archivada en expediente/cancelada) se calcula separado de las etiquetas de Firma y de Expediente, para que ambas dimensiones se lean con claridad sin mezclarse. Mergeado en producción en la Fase 3I-2.
- **`ordenesCompra[]` es de escritura admin-only**, confirmado empíricamente en `data_sanitize.js` — un `empleado` no puede modificar el arreglo de OCs de un proyecto (se preserva del original). Esto determinó que las acciones de cambio de estado de OC en la UI sean **admin-only por ahora** (ver pendiente #1 en `ROADMAP_LICITAPRO.md` si se decide abrirlo a empleados).
...
- **Mobile-first real, no CSS sobre desktop comprimido.** Se determinó, después de varias iteraciones fallidas, que compactar/agrandar el markup de escritorio no resuelve la UX móvil — hace falta **markup dedicado** (`.mobile-only` / `.desktop-only`) para las vistas más densas (Dashboard, Proyectos, Centro de Control, Operación/OC, modal de OC, Cotización, Cotización Operativa, Inbox). **Rediseño completo mergeado en producción en la Fase 3I-2**, validado visualmente en dispositivo real antes del merge.
